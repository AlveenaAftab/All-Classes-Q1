...
function sume (){let num1 :number = 12;
  let num2 :number = 12;
  let y=num1+num2;
  console.log(y);
}
...
//in last ()empty we can write values or leave them empty
 function sumes ( num11 ,num22){          //we can write saparate num1 and num2 or also in ()
  let y = num11+num22 ; //(contain variables)
  console.log(y);
}
   sumes(2,5); 
//these values in () called function parameters and function calling in last function calling (2,5) are arguments
//Advance function
function Bio1(Name:string ="Alveena" , Age:number = 21 , Profession:string = "Microbiologist"){
  console.log((`My name is ${Name}`));
  console.log((`My age is ${Age}`));
  console.log((`I am a ${Profession}`));
}
Bio1(); //in these we store valuse when we havenot store in above paranthsis
//or
function Bio(Name1 , Age1 , Profession1){
  console.log((`My name is ${Name1}`));
  console.log((`My age is ${Age1}`));
  console.log((`I am a ${Profession1}`));
}
Bio1("Alveena",21,"Microbiologist"); //store in accornding to above sequence
...
//comes but in this case paranthesis do not comes and we write (=) then define (parameters) and use =>  ,shortest form of function
let sumu=(num1, num2)=>{
    let y =num1+num2
   return y;                 //marzi h return use kre ya console
 }
 console.log(sumu(2,3));
...
//declare means give name to a variable
//initialize means to call and give value
